...
class Sales_order extends CORE_Controller
{
    ...
    function __construct() {
        ...
        $this->load->model('Sales_order_printing_model');
    }
    ...
    function transaction($txn = null,$id_filter=null) {
        switch ($txn){
            case 'list':  //this returns JSON of Issuance to be rendered on Datatable
                ...
                $additional = " AND DATE(sales_order.date_order) BETWEEN '$tsd' AND '$ted'";
                $response['data']=$this->response_rows(
                    'sales_order.is_active=TRUE AND sales_order.is_deleted=FALSE'.($id_filter==null?'':' AND sales_order.sales_order_id='.$id_filter).''.($stats==0?'':' AND sales_order.order_status_id='.$stats).''.($additional==null?'':$additional),
            ...
            case 'create':
                $m_sales_order=$this->Sales_order_model;
                $m_sales_order_printing=$this->Sales_order_printing_model;
                ...
                $m_sales_order->commission=$this->get_numeric_value($this->input->post('commission',TRUE));
                $m_sales_order->down_payment=$this->get_numeric_value($this->input->post('down_payment',TRUE));
                $m_sales_order->terms=$this->input->post('terms',TRUE);
                $m_sales_order->production_time=$this->input->post('production_time',TRUE);
                $m_sales_order->estimated_date=date('Y-m-d',strtotime($this->input->post('estimated_date',TRUE)));
                ...
                // Inserting Sales Order Printing
                $m_sales_order_printing->sales_order_id= $sales_order_id;

                // ## Color for Print (1)
                $m_sales_order_printing->cfp1_opp_adhesive=$this->input->post('cfp1_opp_adhesive',TRUE);
                $m_sales_order_printing->cfp1_adhesive=$this->input->post('cfp1_adhesive',TRUE);
                $m_sales_order_printing->cfp1_cardboard=$this->input->post('cfp1_cardboard',TRUE);
                $m_sales_order_printing->cfp1_paper_cpp=$this->input->post('cfp1_paper_cpp',TRUE);
                $m_sales_order_printing->cfp1_pet_cpp=$this->input->post('cfp1_pet_cpp',TRUE);
                $m_sales_order_printing->cfp1_mat_slipper=$this->input->post('cfp1_mat_slipper',TRUE);
                $m_sales_order_printing->cfp1_other=$this->input->post('cfp1_other',TRUE);

                // ## Description
                $m_sales_order_printing->desc_opp_adhesive=$this->input->post('desc_opp_adhesive',TRUE);
                $m_sales_order_printing->desc_adhesive=$this->input->post('desc_adhesive',TRUE);
                $m_sales_order_printing->desc_cardboard=$this->input->post('desc_cardboard',TRUE);
                $m_sales_order_printing->desc_paper_cpp=$this->input->post('desc_paper_cpp',TRUE);
                $m_sales_order_printing->desc_pet_cpp=$this->input->post('desc_pet_cpp',TRUE);
                $m_sales_order_printing->desc_mat_slipper=$this->input->post('desc_mat_slipper',TRUE);
                $m_sales_order_printing->desc_other=$this->input->post('desc_other',TRUE);

                // ## Quantity (1)
                $m_sales_order_printing->qty1_opp_adhesive=$this->get_numeric_value($this->input->post('qty1_opp_adhesive',TRUE));
                $m_sales_order_printing->qty1_adhesive=$this->get_numeric_value($this->input->post('qty1_adhesive',TRUE));
                $m_sales_order_printing->qty1_cardboard=$this->get_numeric_value($this->input->post('qty1_cardboard',TRUE));
                $m_sales_order_printing->qty1_paper_cpp=$this->get_numeric_value($this->input->post('qty1_paper_cpp',TRUE));
                $m_sales_order_printing->qty1_pet_cpp=$this->get_numeric_value($this->input->post('qty1_pet_cpp',TRUE));
                $m_sales_order_printing->qty1_mat_slipper=$this->get_numeric_value($this->input->post('qty1_mat_slipper',TRUE));
                $m_sales_order_printing->qty1_other=$this->get_numeric_value($this->input->post('qty1_other',TRUE));

                // ## Direct Print
                $m_sales_order_printing->dp_opp_adhesive=$this->input->post('dp_opp_adhesive',TRUE);
                $m_sales_order_printing->dp_adhesive=$this->input->post('dp_adhesive',TRUE);
                $m_sales_order_printing->dp_cardboard=$this->input->post('dp_cardboard',TRUE);
                $m_sales_order_printing->dp_paper_cpp=$this->input->post('dp_paper_cpp',TRUE);
                $m_sales_order_printing->dp_pet_cpp=$this->input->post('dp_pet_cpp',TRUE);
                $m_sales_order_printing->dp_mat_slipper=$this->input->post('dp_mat_slipper',TRUE);
                $m_sales_order_printing->dp_other=$this->input->post('dp_other',TRUE);

                // ## Colors for Print (2)
                $m_sales_order_printing->cfp2_opp_adhesive=$this->input->post('cfp2_opp_adhesive',TRUE);
                $m_sales_order_printing->cfp2_adhesive=$this->input->post('cfp2_adhesive',TRUE);
                $m_sales_order_printing->cfp2_cardboard=$this->input->post('cfp2_cardboard',TRUE);
                $m_sales_order_printing->cfp2_paper_cpp=$this->input->post('cfp2_paper_cpp',TRUE);
                $m_sales_order_printing->cfp2_pet_cpp=$this->input->post('cfp2_pet_cpp',TRUE);
                $m_sales_order_printing->cfp2_mat_slipper=$this->input->post('cfp2_mat_slipper',TRUE);
                $m_sales_order_printing->cfp2_other=$this->input->post('cfp2_other',TRUE);

                // ## Model / Ml
                $m_sales_order_printing->ml_opp_adhesive=$this->input->post('ml_opp_adhesive',TRUE);
                $m_sales_order_printing->ml_adhesive=$this->input->post('ml_adhesive',TRUE);
                $m_sales_order_printing->ml_cardboard=$this->input->post('ml_cardboard',TRUE);
                $m_sales_order_printing->ml_paper_cpp=$this->input->post('ml_paper_cpp',TRUE);
                $m_sales_order_printing->ml_pet_cpp=$this->input->post('ml_pet_cpp',TRUE);
                $m_sales_order_printing->ml_mat_slipper=$this->input->post('ml_mat_slipper',TRUE);
                $m_sales_order_printing->ml_other=$this->input->post('ml_other',TRUE);

                // ## Quantity (2)
                $m_sales_order_printing->qty2_opp_adhesive=$this->get_numeric_value($this->input->post('qty2_opp_adhesive',TRUE));
                $m_sales_order_printing->qty2_adhesive=$this->get_numeric_value($this->input->post('qty2_adhesive',TRUE));
                $m_sales_order_printing->qty2_cardboard=$this->get_numeric_value($this->input->post('qty2_cardboard',TRUE));
                $m_sales_order_printing->qty2_paper_cpp=$this->get_numeric_value($this->input->post('qty2_paper_cpp',TRUE));
                $m_sales_order_printing->qty2_pet_cpp=$this->get_numeric_value($this->input->post('qty2_pet_cpp',TRUE));
                $m_sales_order_printing->qty2_mat_slipper=$this->get_numeric_value($this->input->post('qty2_mat_slipper',TRUE));
                $m_sales_order_printing->qty2_other=$this->get_numeric_value($this->input->post('qty2_other',TRUE));
                $m_sales_order_printing->save();
            ...
            case 'update':
                ...
                $m_sales_order_printing->delete_via_fk($sales_order_id); 
    ...
    function response_rows($filter_value){
        ...
        return $this->Sales_order_model->get_list(
            ...
            array(
                ...
                array('customers','customers.customer_id=sales_order.customer_id','left'),
                array('order_status','order_status.order_status_id=sales_order.order_status_id','left'),
                array('sales_order_printing','sales_order_printing.sales_order_id=sales_order.sales_order_id','left')
            ),
            'sales_order.sales_order_id DESC'
