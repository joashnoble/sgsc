...
class Sales_order_item_model extends CORE_Model
{
    ...
    function get_products_with_balance_qty_issued($sales_order_id){
    ...
    $sql="SELECT o.*,(o.line_total-o.non_tax_amount)as tax_amount FROM
            ...
            /* END OF EDIT FOR COMPUTATION FOR SALES ORDER AS BUTTON ACCEPT IS CLICKED */          
                ...
                (
                    ...
                    0 as SoQty,
                    SUM(iditems.issue_qty) as InvQty,
                    0 as so_price,
                    0 as so_discount,
                    0 as so_tax_rate,
                    iditems.batch_no,
                    ...
                    FROM (issuance_department_info as idinfo
                    INNER JOIN sales_order as so ON idinfo.sales_order_id=so.sales_order_id)
                    INNER JOIN issuance_department_items as iditems ON idinfo.issuance_department_id=iditems.issuance_department_id
                    WHERE so.sales_order_id=$sales_order_id AND idinfo.is_active=TRUE AND idinfo.is_deleted=FALSE
                    GROUP BY so.so_no,iditems.product_id
