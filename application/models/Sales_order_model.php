...
function get_issued_balance_qty($id){
        $sql="SELECT SUM(x.Balance)as Balance
        FROM
        (SELECT
        m.sales_order_id,
        ...
        FROM
        (issuance_department_info as idinfo
        INNER JOIN sales_order as so ON idinfo.sales_order_id = so.sales_order_id)
        INNER JOIN issuance_department_items iditems ON idinfo.issuance_department_id = iditems.issuance_department_id
        WHERE so.sales_order_id = $id AND idinfo.is_active = TRUE AND idinfo.is_deleted = FALSE
