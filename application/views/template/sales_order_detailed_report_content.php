...
<html>
<head>
	<title>Customer Sales Order Report (Detailed)</title>
	<style>
		body {
...
<body>
    ...
    <div class="">
        <h3><strong>SALES ORDER REPORT PER CUSTOMER (DETAILED)</strong></h3>
        <strong> PERIOD : <?php echo $_GET['startDate'] .' - '.$_GET['endDate']; ?></strong>
    </div>
		...
		<table width="100%" border="1" cellspacing="0" cellpadding="2">
			...
			<?php foreach($sales_details as $sales_detail) { ?>
				<?php if ($customer->customer_id == $sales_detail->customer_id) { ?>
					<tr>
						<td><?php echo $sales_detail->so_no; ?></td>
						<td><?php echo date('Y-m-d', strtotime($sales_detail->date_order)); ?></td>
						<td><?php echo $sales_detail->product_code; ?></td>
						<td><?php echo $sales_detail->product_desc; ?></td>
						<td align="right"><?php echo number_format($sales_detail->so_price,2); ?></td>
						<td align="right"><?php echo $sales_detail->so_qty; ?></td>
						<td align="right"><?php echo number_format($sales_detail->total,2); ?></td>
					</tr>
					<?php $sum += $sales_detail->total;  ?>
				<?php } ?>
			<?php } ?>
			<tr>
